...
static const char kodJavaScript[] = R"=====(XHR = function()
{
	...
	this.send_form = function(form,callback,extra_values) {
		...
		for (var i = 0; i < form.elements.length; i++)
		{
			...
			if (e.options) code += (code ? '&' : '') +	form.elements[i].name + '=' + encodeURIComponent(e.options[e.selectedIndex].value);
			else if (e.length)
			{
				for (var j = 0; j < e.length; j++)
					if (e[j].name) 	code += (code ? '&' : '') + e[j].name + '=' + encodeURIComponent(e[j].value);
			}
			else code += (code ? '&' : '') +	e.name + '=' + encodeURIComponent(e.value);
		}
...
void HttpPageXhrJs(HttpServer *server, WiFiClient& client)
{
    uint len = strlen(kodJavaScript);
    
    Serial.print("Content-Length:");
    Serial.println(len);

    client.printf( "HTTP/1.1 200 OK\r\nContent-type:application/x-javascript; charset=utf-8\r\nContent-Length:%i\r\nConnection: close\r\n\r\n", len);
    client.print(kodJavaScript);
